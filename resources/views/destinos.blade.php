    ...
    <main class="container d-flex flex-column gap-3">
        ...
        @foreach ($destinos as $destino)
            <article
                ...
                <a class="align-self-start position-absolute p-2 rounded link">{{ $destino->FEC_PREV }} /
                    {{ $destino->DIAS_PREV }}</a>
                ...
                <p class="align-self-end px-3"><b>Precio estimado: </b><span
                        class="boton_fecha rounded py-1 px-2">{{ $destino->PRE }}€</span></p>
                <p class="align-self-end px-3"><b>Precio estimado/noche: </b><span class="boton_fecha rounded py-1 px-2">{{ $destino->PRE / $destino->DIAS_PREV  }}€</span></p>
            </article>
        @endforeach

    </main>
    <nav id="menu_principal"
        class="bg-dark w-100 fixed-bottom text-light d-flex justify-content-around">
